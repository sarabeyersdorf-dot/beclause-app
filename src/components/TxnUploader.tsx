...
const SUPA_URL = V.VITE_SUPABASE_URL || process.env.NEXT_PUBLIC_SUPABASE_URL;
const SUPA_ANON = V.VITE_SUPABASE_ANON_KEY || process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;
const MAX_MB = Number(V.VITE_MAX_FILE_MB || process.env.NEXT_PUBLIC_MAX_FILE_MB || 25);

export default function TxnUploader({
  transactionId, checklistItemId,
}: { transactionId: string; checklistItemId?: string }) {
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState<string>();
  const [done, setDone] = useState(false);

  if (!supabase) return <p style={{color:'red'}}>Supabase not configured.</p>;

  async function onFile(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > MAX_MB * 1024 * 1024) { setError(`File too large (>${MAX_MB}MB)`); return; }
    ...
    try {
      ...
      const { error: upErr } = await supabase.storage.from('txn-docs')
